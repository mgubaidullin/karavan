...
export const DslMetaApi = {
    ...
    camelizeName: (name: string, separator:string, firstSmall: boolean) => {
        const res =  name.split(separator).map(value => DslMetaApi.capitalizeName(value)).join("");
    ...
    getDslClassByName: (name: string): string | undefined => {
        return DslMetaApi.findDslMetaModelByName(name)?.javaType
    },

    getDslModelInterfaceByName: (name: string): string | undefined => {
        return DslMetaApi.getDslClassByName(name)
            ?.replace("org.apache.camel.", "")
            .split('.').map(s => DslMetaApi.capitalizeName(s)).join("")
    },
    ...
    log: () => {
      // ['choice', 'filter', 'to', 'when', 'otherwise', 'saga', "aggregate"].forEach(value => {
      //     console.log(DslMetaApi.getDslClassByName(value))
      //     console.log(DslMetaApi.getDslModelInterfaceByName(value))
      //     console.log(DslMetaApi.isDslModelHasSteps(value))
      // })
      //   console.log(DslMetaApi.getProcessDefinitionElements(false));
    },

    loadCamelYamlDsl: () => {
        fetch("camel-yaml-dsl.json").then(res => res.json()).then(res => {
                CamelYamlDsl = res;
                DslMetaApi.log();
        })
    },
    ...
    getIcon: (name: string, uri?: string): string => {
